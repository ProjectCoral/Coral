...
class PermSystem:
    ...
    def __init__(self, register, config):
        ...
        self.register.register_command(
            "perms",
            "perms <show|list>|<add|remove> <perm_name> <user_id> <group_id> - Manages user permissions.",
            self.perm_command,
            ["permission_system"],
        )
    ...
    def load_user_perms(self):
        if not os.path.exists(self.perm_file):
            logger.warning(
                "[yellow]Permission file not found, creating a default one.[/]"
            )
    ...
    def check_perm(
        self, perm_name: Union[str, List[str]], user_id: Union[str, int], group_id: Union[str, int]
    ) -> bool:
        if user_id == "Console":
            return True
        if isinstance(perm_name, list):
            ...
            return False
        logger.debug(
            f"Checking permission {perm_name} for user {user_id} in group {group_id}"
        )
        if perm_name not in self.registered_perms:
            logger.warning(
                f"[yellow]Permission {perm_name} not registered, ingoring it.[/]"
            )
            return True
        user_id = str(user_id)
        ...
        if user_id == "-1":
            ...
            if user_id not in self.user_perms:
                logger.warning(
                    f"[yellow]User {user_id} has no permissions registered.[/]"
                )
                return False
    ...
    def show_perms(self, *args):
        message = (
            "Total registered " + str(len(self.registered_perms)) + " Permissions.\n"
        )
