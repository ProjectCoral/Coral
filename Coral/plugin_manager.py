...
class PluginManager:
    ...
    async def load_all_plugins(self):
        ...
        self.register.register_command(
            "reload", "Reload all plugins", self.reload_command, ["pluginmanager"]
        )
    ...
    async def load_plugin(self, plugin_name):
        ...
        if os.path.exists(requirements_file):
            if not os.path.exists(
                requirements_file + ".coral_installed"
            ) and not await self.register.execute_function(
                "check_pip_requirements", requirements_file
            ):
                logger.warning(
                    "[yellow]Plugin [/]"
                    + str(plugin_name)
                    + "[yellow] has unmet requirements, trying to install them...[/]"
                )
                if not await self.register.execute_function(
                    "install_pip_requirements", requirements_file
                ):
                    logger.error(
                        "[red]Failed to install requirements for plugin [/]"
                        + str(plugin_name)
                        + "[red] , skipping...[/]"
                    )
                    return None
            ...
            if meta:
                if (
                    ...
                    logger.error(
                        "[red]Plugin [/]"
                        + str(plugin_name)
                        + "[red] is not compatible with this version of the Plugin Manager, skipping...[/]"
                    )
                    return None
            else:
                logger.warning(
                    "[yellow]Plugin [/]"
                    + str(plugin_name)
                    + "[yellow] did not provide a compatibility check, which is deprecated and might be broken in a future version.[/]"
                )
                ...
                for target in node.targets:
                    if isinstance(target, ast.Name) and target.id == "__plugin_meta__":
                        ...
                        if isinstance(value_node, ast.Dict):
                            # 尝试将字典结构转换为字典对象
                            keys = [
                                k.s for k in value_node.keys if isinstance(k, ast.Str)
                            ]
                            values = [
                                (
                                    v.s
                                    if isinstance(v, ast.Str)
                                    else v.n if isinstance(v, ast.Num) else repr(v)
                                )
    ...
    async def show_plugins(self, *args):
        return (
            "Available plugins:\n"
            + str(self.plugins)
            + "\n Running Plugin Manager Version "
            + self.pluginmanager_version
            + "\n"
        )
    ...
    async def reload_command(self, *args, **kwargs):
        ...
        end_time = time.time()
        logger.warning(
            f"[yellow]It's not recommended to reload frequently, as it can cause issues. Please use the reload command only when necessary.[/]"
        )
